...
Spiders are *supposed* to respect rules set in `robots.txt` files, but they sometimes ignore them (either through malice or by mistake), so it's also useful to block them entirely.
...
in {
  "vhost-A" = {
    # ... other config ...
    locations."=/robots.txt".alias = ${robots_txt};
    extraConfig = ''
